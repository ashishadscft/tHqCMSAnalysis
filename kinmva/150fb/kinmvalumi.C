...
qw(hat);
cout<<"bins x" <<hat->GetNbinsX()<<endl;
cout<<"bins"<<hmc->GetNbinsX()<<endl;
////////////////////////////////////////////
...
int minXbin = 1, maxXbin = hmc->GetNbinsX();
for (int i=1; i <= hmc->GetNbinsX(); i++) {
   if (hmc->GetBinContent(i)) {
      minXbin = i;
      break;
   }
}
for (int i=hmc->GetNbinsX(); i >= 1; i--) {
   if (hmc->GetBinContent(i)) {
      maxXbin = i;
      break;
   }
...
cout<<"min max" <<minXbin<<maxXbin<<endl;
cout<<"integral total" <<hat->Integral()<<endl;
/////////////////////////////////////////////
...
hat->SetTitle("CMs preliminary Test               150f/b");
