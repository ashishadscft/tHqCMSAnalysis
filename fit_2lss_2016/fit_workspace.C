...
//float(efakes)=(i1)*(eps_fakes*alpha_fakes->getError()); //fakes eps=0.4
//float(etz)=(i2)*(eps_tz*alpha_tz->getError());  //tz    eps=0.5
//float(ewz)=(i3)*(eps_wz*alpha_wz->getError()); //wz   eps=0.5
//float(ettz)=(i4)*(eps_ttz*alpha_ttz->getError()); //ttz  double  eps_ttz = sqrt(pow(0.04,2)+pow(0.1,2));
//float(ettw)=(i5)*(eps_ttw*alpha_ttw->getError()); //ttw   double eps_ttw= sqrt(pow(0.04,2)+pow(0.12,2));
//float(eth)=(i6)*(alpha_mu->getError());  //signal (kt--1 mu=6.12861e-01)
//float(etth)=(i7)*(eps_tth*alpha_tth->getError()); //tth    eps=0.05
