\chapter{Statistical Analysis}
	...
	The fit is applied by minimizing the $-\log{L}$ (NLL) with respect to the parameter $\mu$ and $\alpha_k$. The minimization is performed by using programming language ROOT. ROOT is a scientific software toolkit, based on the C++ language, which main objective is the handling of data processing, statistical analysis, including visualization and storage. The package ROOFIT is used here, because of its implementations of modeling probability distributions and statistical tools\cite{roofit}. Figure \ref{simple} and Table \ref{table1}, shows the results of the model fitting using the Asimov data for both models. 
 ...
 Table \ref{parameters}, shows the $\mu$ and $\alpha$ parameters. For the pre-fit, values of $\alpha$ are set to zero and $\mu$ is set to 1. After the fit, $\alpha$ parameters set to zero indicates that in the fit, the values of the backgrounds didn't change for the SM and the $\mu$ parameter set to 1 indicates that the signal strength also didn't change.
	...
	It is useful to scan $q$ as function of $\mu$, which is normally a parabolic function as shown in figure \ref{scanl}. In the SM case, the scan has a wider shape in comparison to the $k_t$=-1. This is due to larger statistical uncertainty on the $\mu$ parameter. 
	...
	In the $k_t$=-1 model the uncertainty of $\mu$ decreases with higher luminosities giving better results. This is caused by the fact that the number of events for the $tH$ signal is greater. For 3000 fb$^{-1}$ the uncertainty on $\mu$ is $~10\%$, and it has an upper limit that rounds between 0.9-1.1. In the likelihood scan, the statistical test $q$ gets very high values at $\mu$>1.1. This indicates that for large integrated luminosities this model can be fully ruled out. This can be visualized in the likelihood scan of  3000 fb$^{-1}$, where the curve is very closed and values of $q$ are very high, giving incompatibility of the model with data.
	...
	\end{table}
		...
		\caption{Illustration of the $\chi^2$ distribution with number of degrees of freedom of 7 used for the limit estimation}
		\label{chi}
	\end{figure}
	...
	\begin{figure}[ht]
		...
		\hfill
		\begin{minipage}[b]{0.48\textwidth}
			\includegraphics[width=\textwidth]{Chapter4/150fb/Likelihood.png}
		\end{minipage}
		\begin{minipage}[b]{0.48\textwidth}
		...
		\hfill
		\begin{minipage}[b]{0.48\textwidth}
			\includegraphics[width=\textwidth]{Chapter4/300fb/Likelihood.png}
		\end{minipage}
		\begin{minipage}[b]{0.48\textwidth}
