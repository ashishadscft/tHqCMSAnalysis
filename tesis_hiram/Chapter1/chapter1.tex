...
Baryons are fermions made of 3 quarks. Examples of baryons are protons and neutrons. Both types of particles can interact via the strong force. Since quarks and gluons only exist in bound, it is impossible to observe isolated quarks except the top quark, which decays before it has time to form a
...
There are experiments that detect particles around the world, such as the large hadron collider (LHC) in Europe, the Fermilab in United States of America, KEK in Japan, etc. More information about the LHC will be described in the next chapters. 
	...
	\begin{figure}[ht]
		...
		\caption{Particle interactions in Standard model \footnote{Taken from \url{https://commons.wikimedia.org/wiki/File:Elementary_particle_interactions_in_the_Standard_Model.png}. Created by Eric Drexler.}}
		\label{sm}
	\end{figure}
