...
\begin{center}
    ...
    \end{figure}
\end{center}
\footnotetext{Taken from Wikimedia. \small \url{https://upload.wikimedia.org/wikipedia/commons/0/00/Standard_Model_of_Elementary_Particles.svg}}
...
$W^i$ (i=1,2,3) are three  SU(2) gauge bosons and B is a U(1) gauge boson.\footnote{$\epsilon^{ijk}$ represents the Levi-Civita tensor, where $\epsilon^{ijk}= \begin{dcases}
	...
	\end{dcases}$.} In the end, those gauge bosons, after introduce the Higgs mechanism, become the $W^\pm$ , $Z$ and $\gamma$ vector boson \cite{ew1,ew2}. %Z bosons  and photons will be formed by a combination of W and B fields.\\
\\
...
\caption{
Cross section for different processes generated in a pp collision} \label{csp}
\end{figure}
