...
These  datasets are processed using computing clusters which use high-performance computing systems, normally using \textsc{Condor}~\cite{ref:condor} to link the processing nodes, which allows hundreds of jobs to be run at the same time by individual users and save the output in \textsc{Root} format \cite{ref:root}.
...
These backgrounds must be determined from MC simulation and normalized to their predicted SM crossections. 
...
A typical algorithm used is the Boosted Decision Tree~\cite{ref:tmva}, which takes as input several event variables related to the signal topology.
...
A fit is applied using the \textsc{RooFit} software package~\cite{ref:roofit} which constructs a model for the data consisting of the sum of the backgrounds and the signal simulations. A likelihood function is constructed from a Poison function using the expected and measured number of events.
Systematic uncertainties are incorporated as nuissance parameters using gaussian probability functions, these account for uncertaintes in the normalization and shapes of the background.
...
\item A presentation of a poster or a talk in one of the national conferences of the Division of Paticles and Fields of the Mexican Society of Physics.
\item A presentation of a poster or a talk in an international physics conference.
