...
The currently published result uses 36 fb$^{-1}$ of collision data and obtains an upper limit of 2.04 pb while only 0.077 pb are expected in the SM \cite{ref:tH2019,ref:tHMultilep}, the limit is 26 times larger than the expected value.
...
The reconstruction and selection of signal events is studied using the \textsc{MadGraph}~\cite{ref:madgraph} Monte Carlo generator which implements the theoretical model and predict the amount and distribution of events for a given LHC dataset.
...
A separate simulation of the detector is employed using \textsc{GEANT}~\cite{ref:geant} which simulates the trajectory of the stable particles and the response and efficiency of each sensor in the CMS detector.
